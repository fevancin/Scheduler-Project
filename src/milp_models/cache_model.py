...
from src.common.custom_types import CacheMatch, Window


def get_cache_model(
        instance: MasterInstance,
        cache: Cache,
        best_cache_result_value_so_far: float | None) -> pyo.ConcreteModel:
    ...
    request_index = set((r.patient_name, r.service_name, r.window.start, r.window.end) for r in cache.keys())

    model.choice_index = pyo.Set(initialize=sorted(choice_index)) # type: ignore
    model.request_index = pyo.Set(initialize=sorted(request_index)) # type: ignore

    del day_names, choice_index, request_index
    ...
    model.request = pyo.Var(model.request_index, domain=pyo.Binary) # type: ignore

    # VINCOLI ##################################################################

    # Se una richiesta è soddisfatta, è soddisfatta da almeno un giorno ed una
    # iterazione interna alla sua finestra
    @model.Constraint(model.request_index) # type: ignore
    def link_choose_to_window_variables(model, p, s, start, end):
        ...
        return pyo.quicksum(model.choose[i, d] for i, d in affected_tuples) >= model.request[p, s, start, end]
    ...
    model.objective_function_value = pyo.Var(domain=pyo.NonNegativeReals) # type: ignore

    @model.Constraint() # type: ignore
    def build_objective_function_value(model):
        return pyo.quicksum(model.request[p, s, start, end] * instance.services[s].duration * instance.patients[p].priority for p, s, start, end in model.request_index) == model.objective_function_value
    
    # La variabile 'objective_function_value' non può essere peggiore del
    # migliore risultato della cache fino ad ora. Attenzione che questa aggiunta
    # causa per qualche motivo delle soluzioni non feasible.
    # if best_cache_result_value_so_far is not None:
    #     @model.Constraint() # type: ignore
    #     def no_worse_solutions(model):
    #         return model.objective_function_value >= best_cache_result_value_so_far
